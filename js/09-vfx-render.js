...
function _playerMorphApply() {   // 8fps ticker 驅動（🗡️ v3.0.67 形態＝變身優先→職業動態·key 含武器變體→換武器自動重建）
    ...
    if (_pmState.act) {
        ...
        if (seq) {
            let _fms = (_pmState.act === 'attack') ? _atkFrameMs((player.d && player.d.aspd) || 0, seq.length) : (1000 / MOB_ANIM_FPS);   // ⚔️ v3.0.91 攻擊動畫隨攻速加速
...
function _allySpritesApply() {   // 8fps ticker 驅動（先於 _playerMorphApply→_partyBottoms 供主玩家消費）
    ...
    allies.forEach((ally, i) => {
        ...
        if (st.act) {
            ...
            if (seq) {
                let _fms = (st.act === 'attack') ? _atkFrameMs((typeof atkSpdBaseItv === 'function' ? atkSpdBaseItv(ally) : 0), seq.length) : (1000 / MOB_ANIM_FPS);   // ⚔️ v3.0.91 傭兵攻擊動畫隨攻速加速（用其實際排程間隔 atkSpdBaseItv）
